...
signal open_diplomacy_requested
signal open_tech_requested
signal open_military_requested
signal open_minister_requested
...
func _build_ui() -> void:
	# 原 _create_framework_hub：根节点即中枢容器（原 _framework_hub 的职责由 self 承担）
...
func _open_player_capital_panel() -> void:
	...
	# 打开动作交由 main 调度
	emit_signal("open_city_requested", str(capital.get("id", "")))
...
func _hide_framework_placeholder(user_closed: bool = true) -> void:
	if is_instance_valid(_framework_placeholder_layer):
		_framework_placeholder_layer.visible = false
	# 恢复中枢/工具栏的时机由 main 判断（大地图开着时不恢复）
	if user_closed:
		emit_signal("hub_visibility_changed", true)
...
func _open_formal_minister_panel() -> void:
	emit_signal("open_minister_requested")
