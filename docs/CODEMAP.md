...
> 更新于 2026-09-14，供快速定位文件职责和关键函数。
> 实现状态见 `docs/程序进度/程序已实现功能.md`；机制以 `docs/机制概览/` 专项文档为准。
> 一键回归：`tools/run_auto_regression.ps1`；流程冒烟：`tests/smoke/full_flow_smoke.gd`。
...
| 目录 | 内容 |
|------|------|
| docs/ | 根入口：README、CODEMAP、六角网格规范 |
| docs/机制概览/ | 完整游戏机制设计（权威指引） |
| docs/程序进度/ | 已实现功能清单、审计差异 |
| docs/测试指南/ | 演武场景测试指南、Demo 封版冒烟清单 |
| docs/美术进度/ | 美术交付清单与程序交付说明 |
| docs/协作/ | 当前使用中的中文协作入口、模板与清单 |
| docs/归档/策划决策/ | 阶段 0~7 决策记录（溯源） |
| docs/归档/过程稿/ | 计划选型、交接快照、旧规范、历史 PROMPT |
| docs/归档/其他 | 英文协作参考、历史交付重复副本 |
